        ...
        private void HandleDataReceived(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                ProcessReceiveBuffer(handler, line =>
                {
                    ...
                    if (command == PacketTypes.RelyMessage && parts.Length >= 5)
                    {
                        if (!handler.IsLoggedIn)
                        {
                            SendTo(handler, PacketBuilder.BuildLoginFail("Bạn chưa đăng nhập"));
                        ...
                        string replyToId = parts[2];
                        string replyToSender = parts[3];

                        string safePacket = PacketBuilder.BuildReplyMessage(handler.Username, replyContent, replyToId, replyToSender);
                        string display = $"{handler.Username} (trả lời {replyToSender}): {replyContent}";
                        OnMessageReceived?.Invoke(display);
                        BroadcastToLoggedIn(safePacket);
                        return;
                    }
                    if (command == PacketTypes.FowardMessage && parts.Length >= 4)
                    {
                        if (!handler.IsLoggedIn)
                        {
                            SendTo(handler, PacketBuilder.BuildLoginFail("Bạn chưa đăng nhập"));
                        ...
                        string safePacket = PacketBuilder.BuildForwardMessage(handler.Username, content, originalSender);
                        string display = $"{handler.Username} (chuyển tiếp từ {originalSender}): {content}";
                        OnMessageReceived?.Invoke(display);
                        BroadcastToLoggedIn(safePacket);
                    }
                    if (command == "EMOJI_REACTION" && parts.Length >= 3)
                    {
                        if (!handler.IsLoggedIn)
                        {
                            SendTo(handler, PacketBuilder.BuildLoginFail("Bạn chưa đăng nhập"));
                        ...
                        string safePacket = $"EMOJI_REACTION|{handler.Username}|{emoji}|{messageId}";
                        string display = $"{handler.Username} đã phản ứng với {messageId} bằng {emoji}";
                        OnMessageReceived?.Invoke(display);
                        BroadcastToLoggedIn(safePacket);
                        return;
                    }
